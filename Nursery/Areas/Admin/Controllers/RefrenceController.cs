...
namespace Nursery.Areas.Admin.Controllers
{
    ...
    public class RefrenceController : Controller
    {
        ...
        public async Task<IActionResult> ActiveDisable(int indexN, int isEnded)
        {
            List<TblRefrence> refrence = _db.Refrence.Get(i => i.IndexNo == indexN).ToList();
            foreach (var item in refrence)
            {
                if (isEnded == 1)
                {
                ...
                _db.Refrence.Update(item);
            }
            _db.Save();
            return await Task.FromResult(Redirect("Refrence"));
        }
    }
}
